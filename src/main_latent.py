'''Credit: This script extends code from the CUTS repo ("https://github.com/ChenLiu-1996/CUTS").'''
import argparse
import numpy as np
import torch
import yaml
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gray', action='store_true',
                        help="If set, optimize latent space over grayscale data")
    parser.add_argument('--cfg-path', type=str, default="config/",
                        help="Path to config folder")
    parser.add_argument('--mode', help='`train` or `test`?', required=False, default='train')

    args = vars(parser.parse_args())
    args = AttributeHashmap(args)
    cfg_file_name = f"dmr_cfg{'_gray' if args.gray else ''}.yaml"
